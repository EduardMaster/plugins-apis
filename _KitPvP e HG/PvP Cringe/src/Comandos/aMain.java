...
public class aMain extends WarpsAPI implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (KitAPI.getKit(p) != "Nenhum") {
			p.sendMessage("§6§lMAIN §fVoce ja pegou um kit!");
			return true;
		}
		if (KitAPI.getKit(p) == "Nenhum") {
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstace(), new Runnable() {
				...
				public void run() {
					...
					p.sendMessage("§6§lMAIN §fVoce entrou na arena Main!");
					BarAPI.setMessage(p, "§6§lMAIN §fVoce entrou na arena Main", 3);
			...
			if (args[0].equalsIgnoreCase("set")) {
			    if (p.hasPermission("cmd.dono")) {
			    	Set(p, "main");
			    	p.sendMessage("§6§lMAIN §fVoce setou a arena main!");
			}
		  }
		}
		return false;
