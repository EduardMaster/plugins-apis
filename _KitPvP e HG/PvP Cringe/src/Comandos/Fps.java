...
public class Fps extends WarpsAPI implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (KitAPI.getKit(p) != "Nenhum") {
			p.sendMessage("§6§lFPS §fVoce ja pegou um kit!");
			return true;
		}
		if (KitAPI.getKit(p) == "Nenhum") {
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstace(), new Runnable() {
				...
				public void run() {
					...
					p.sendMessage("§6§lFPS §fVoce entrou na arena FPS!");
					BarAPI.setMessage(p, "§6§lFPS §fVoce entrou na arena FPS", 3);
			...
			if (args[0].equalsIgnoreCase("set")) {
			    if (p.hasPermission("cmd.dono")) {
			    	Set(p, "Fps");
			    	p.sendMessage("§6§lFPS §fVoce setou a arena FPS!");
			}
		  }
		}
		return false;
