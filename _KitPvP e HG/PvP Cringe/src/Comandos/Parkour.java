			...
			if (KitAPI.getKit(p) == "Nenhum") {
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstace(), new Runnable() {
					...
					public void run() {
						...
						p.sendMessage("§8§lPARKOUR §fVoce entrou no Parkour");
						BarAPI.setMessage(p, "§8§lPARKOUR §f oce entrou no Parkour", 3);
				...
				if (args[0].equalsIgnoreCase("set")) {
				    if (p.hasPermission("cmd.dono")) {
				    	Set(p, "Parkour");
				    	p.sendMessage("§8§lPARKOUR §fVoce setou o Parkour!");
				}
			  }
			}
			return false;
