...
public static void announceFeast(Integer time) {
  ...
  public static void spawnFeast() {
    ...
    Bukkit.getLogger().info("O Feast nasceu em §f" + df.format(mainBlock.getLocation().getX()) + "," + df.format(mainBlock.getLocation().getY()) + "," + df.format(mainBlock.getLocation().getZ()));
    Bukkit.broadcastMessage("§7[§a!§7] §a§lO BONUS FEAST NASCEU!");
    ChecarVitoria.ProtecaoFeast = false;
    BonusFeast.spawned = false;
