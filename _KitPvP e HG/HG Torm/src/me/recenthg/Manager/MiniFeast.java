...
import me.recenthg.Borda.BorderType;
import me.recenthg.Borda.WorldBorder;
...
public static void spawnFeast()
  {
    ...
    Bukkit.broadcastMessage("§b§l» §fUm mini feast nasceu entre §7X: (§e" + 
      df.format(mainBlock.getLocation().getX() + 50.0D) + " §fe §e" + 
      df.format(mainBlock.getLocation().getX() - 50.0D) + "§7) §fe §7Z: (§e" + 
      df.format(mainBlock.getLocation().getZ() + 50.0D) + " §fe§e " + 
      df.format(mainBlock.getLocation().getZ() - 50.0D) + "§7)");
    spawnChests();
  ...
  private static void spawnChests()
  {
    ...
    for (Integer i : co)
    {
      ...
      case -2:
        break;
      default:
        log.warning("§a● §7Algo ocorreu enquanto spawnava os baus: " + 
          i.toString());
      }
      if (i.intValue() == -1)
      {
