...
public class Title {
	...
	public static void sendTitle(Player p, String title) {
		...
		if (MCAlpha.Status == GameTime.PREGAME && title.toLowerCase().contains("commsg")) {
			((CraftPlayer) p).getHandle().playerConnection
					.sendPacket(new ProtocolInjector.PacketTitle(ProtocolInjector.PacketTitle.Action.TITLE,
							ChatSerializer.a("{\"text\": \"\"}").a(title.replace("commsg", ""))));
			p.sendMessage(title.replace("commsg", ""));
		} else {
	...
	public static void sendXPBarMessage(Player v, String mensagem) {

		if (((CraftPlayer) v).getHandle().playerConnection.networkManager.getVersion() < VERSION) {
			return;
		}
		CraftPlayer p = (CraftPlayer) v;

		IChatBaseComponent cbc = ChatSerializer
				.a("{\"text\":\"" + ChatColor.translateAlternateColorCodes('&', mensagem) + "\"}");
